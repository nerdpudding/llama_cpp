...
**v2 — 10 maart 2026**
*Updated from v1 with current European pricing (verified eBay.de, Alternate.nl, DectDirect.nl), a concrete LoRA video training scenario, an investigated-and-rejected server build option, and conclusions specific to my upgrade path. The benchmark data and technical background sections are unchanged from v1.*
...
- Video model training (like LTX2 LoRA) where the current desktop is forced into FP8/512p compromises and agressieve CPU offloading — the Spark enables FP16/720p without offload penalties
...
**The honest answer for my situation:** the Spark fills real, concrete gaps I'm hitting right now — LTX2 LoRA training forced into compromises, large models that don't fit, multi-model pipelines that exceed VRAM. A full platform upgrade (AM5 + Pro 6000 96GB) would be superior in every dimension but costs ~€11,250 and is not financially viable now. A self-built server (EPYC + A100) is not realistic in Europe at consumer level (see [Investigated: Server build](#investigated-server-build-epyc--a100)). The Spark at ~€4,500 is the best available compromise: it solves the immediate AI bottleneck, keeps the gaming PC untouched, and doesn't preclude a PC upgrade later.
...
- [Training and fine-tuning](#training-and-fine-tuning)
  ...
  - [Video model training: LTX2 LoRA (real-world scenario)](#video-model-training-ltx2-lora-real-world-scenario)
- [The hardware alternatives](#the-hardware-alternatives)
  - [Option 1: Add an RTX 5090](#option-1-add-an-rtx-5090-3200-rising)
  - [Option 2: Used professional GPUs](#option-2-used-professional-gpus-a6000a100)
  - [Option 3: Cloud computing](#option-3-cloud-computing)
  - [Option 4: DGX Spark](#option-4-dgx-spark-4500-in-nl)
  - [Option 5: Full platform upgrade (AM5 + Pro 6000 96GB)](#option-5-full-platform-upgrade-am5--pro-6000-96gb)
  - [Investigated: Server build (EPYC + A100)](#investigated-server-build-epyc--a100)
  - [Comparison at a glance](#comparison-at-a-glance)
- [My upgrade path](#my-upgrade-path)
...
| DGX Spark | 20-core ARM (10x Cortex-X925 + 10x Cortex-A725) | Blackwell GB10 (6,144 CUDA cores) | 128 GB unified LPDDR5X | 273 GB/s | 240W external | ~€4,500 in NL (see pricing note) |
...
**DGX Spark pricing note (v2 update):** NVIDIA raised the MSRP from $3,999 to $4,699 in early 2026 due to global memory supply chain issues. In the Netherlands, DectDirect lists the Founders Edition at €4,486 incl. BTW (as of 10 March 2026). Alternate.nl also lists it but pricing was not confirmed. The ASUS Ascent GX10 (1TB SSD variant) is listed at ~$3,000/€2,760 but EU availability is unclear. OEM variants from Dell, HP, and Lenovo exist but detailed EU pricing and stock remain scarce.
...
- **Sebastian Raschka's DGX Spark review:** [First Impressions of the DGX Spark](https://sebastianraschka.com/blog/2025/dgx-impressions.html) by Sebastian Raschka (PhD, author of *Build a Large Language Model (From Scratch)*). Independent hands-on benchmarks covering inference, pre-training, SFT, and DPO training on a 355M model, comparing the DGX Spark against an A100, H100, and Mac Mini M4 Pro. A well-known and respected voice in the ML community — his findings are referenced in the training and conclusions sections below.
...
- **Video generation** — extremely VRAM-hungry. Even short clips can need 20-40+ GB. Currently a struggle on the desktop. See the [LTX2 LoRA training section](#video-model-training-ltx2-lora-real-world-scenario) for a concrete example of hitting this wall.
...
**Independent validation from Sebastian Raschka:** In his [hands-on review](https://sebastianraschka.com/blog/2025/dgx-impressions.html), Sebastian Raschka (author of *Build a Large Language Model (From Scratch)*) trained a 355M parameter model on the DGX Spark across pre-training, supervised fine-tuning (SFT), and DPO preference tuning. His findings: the Spark and A100 were both significantly faster than the Mac Mini M4 Pro across all three training scenarios. For single-sequence inference on a 0.6B model, the Spark performed roughly on par with an H100 — a GPU that costs approximately 6x more. However, Raschka is clear that the Spark is "not a replacement for A100 or H100 GPUs when it comes to large-scale training" and characterizes it as "a development and prototyping system" — a machine for local iteration and small-scale fine-tuning before deploying to cloud GPUs for heavy training. He also noted the Mac Mini ran dangerously hot (over 100°C) during extended evaluation, while the Spark remained office-quiet. This aligns with the findings in this document: the Spark's strength is memory capacity enabling workloads that don't fit elsewhere, not raw compute throughput.
...
### Video model training: LTX2 LoRA (real-world scenario)

*v2 addition — this is a concrete example from my actual workflow, not a hypothetical.*

I'm currently training LTX2 LoRA models for video generation on the RTX 4090. This is where the desktop's limitations become most tangible:

**Current baseline (RTX 4090, 24 GB VRAM + 64 GB DDR4):**

| Parameter | Setting |
|-----------|---------|
| Model | LTX2 LoRA |
| Precision | FP8 (transformers + text encoder) — compromis op accuracy |
| Resolution | 512p (720p geeft OOM) |
| Caching | Transformers latents + text gecached |
| Offloading | Aanzienlijk naar CPU/RAM |
| VRAM gebruik | ~22.5 GB / 24 GB (pieken bij iteraties) |
| RAM gebruik | ~60% van 64 GB (incl. OS) |
| GPU power draw | ~350W (pieken per iteratie) |
| Totaal stroomverbruik | ~500W geschat |
| Snelheid | **8.03 s/it** |
| Trainingsduur (5000 steps) | **~11 uur** |

Every setting here is a compromise forced by 24 GB VRAM. FP8 instead of FP16 means lower training accuracy. 512p instead of 720p means lower resolution output. Aggressive offloading means constant shuffling between GPU and slow DDR4 RAM. And despite all this, the GPU is still near capacity at ~22.5/24 GB.

**What changes with different hardware:**

| | Current desktop | PC + AM5/DDR5 | PC + RTX 5090 | DGX Spark |
|---|---|---|---|---|
| **VRAM** | 24 GB (4090) | 24 GB (4090) | 32 GB (5090) | 128 GB unified |
| **System RAM** | 64 GB DDR4-3600 | 128 GB+ DDR5-6400 | 128 GB+ DDR5-6400 | 128 GB unified |
| **FP16 transformers** | No (OOM) | Partially, via offload | Yes (or FP8 + more room) | Yes |
| **720p resolution** | No (OOM) | Yes, via aggressive offload | Yes | Yes |
| **FP16 + 720p together** | No | Probably not | Yes | Yes |
| **Offloading needed?** | Yes, heavy | Yes, but faster (DDR5) | Minimal | No (unified memory) |

**Estimated training speeds (same FP8/512p settings for fair comparison):**

| Scenario | Est. s/it | vs. current | Est. duration (5000 steps) |
|----------|-----------|-------------|---------------------------|
| Current desktop | 8.0 s/it | baseline | ~11 hours |
| PC + AM5/DDR5 | ~6.5–7.0 s/it | ~15% faster | ~9–10 hours |
| PC + RTX 5090 | ~3–4 s/it | ~2x faster | ~4–6 hours |
| DGX Spark | ~3–5 s/it | ~1.5–2x faster | ~4–7 hours |

**With better settings (FP16/720p — removing the compromises):**

| Scenario | Est. s/it | Feasible? | Note |
|----------|-----------|-----------|------|
| Current desktop | — | ❌ OOM | Not possible with 24 GB VRAM + 64 GB RAM |
| PC + AM5/DDR5 | ~10–12 s/it | ⚠️ Partly | 720p OR FP16, probably not both. Heavy DDR5 offloading |
| PC + RTX 5090 | ~4–5 s/it | ✅ Yes | 32 GB VRAM + faster compute, most combinations fit |
| DGX Spark | ~7–10 s/it | ✅ Yes | Everything fits easily, but GB10 compute is slower than discrete GPU |

**⚠️ These are estimates based on hardware specs and known bottlenecks, not measured benchmarks.**
...
### Option 1: Add an RTX 5090 (~€3,200+, rising)
...
**But:** The 5090 is extremely hard to get at MSRP (~€2,100). Real street prices in the Netherlands/Germany are €3,000-4,000 as of March 2026, with the average hovering around €3,353. Availability at Dutch retailers (Megekko, Coolblue, Alternate) remains minimal — mostly pre-orders without firm delivery dates. A 70B dense model at Q4 is 43 GB — it barely fits on 48 GB and leaves no room for KV cache. Multi-model workflows still overflow to CPU. A large PSU is needed (the 5090 alone draws 575W), plus good airflow and a case that fits it. At €3,200+ for a single GPU, the cost approaches Spark territory but with 3x less memory.

### Option 2: Used professional GPUs (A6000/A100)

More VRAM per card, but significantly more expensive than expected in Europe:

**A6000 (48 GB):** Hard to find as used purchase in NL/DE; mostly available as cloud rental (~€0.30-2.50/hr). Slower than a 4090 for inference (Ampere, not Ada). Older CUDA architecture.

**A100 (80 GB):** Real European prices as of March 2026 (eBay.de):
- A100 80 GB PCIe (SXM4→PCIe conversie, Chinese verkopers): ~€7,600 — no warranty, converted card, questionable origin
- A100 80 GB PCIe (original, new): €15,000–18,000+ — barely available
- A100 80 GB SXM4 (refurbished): ~€6,200 — requires an HGX baseboard (~€3,000+), making total cost €9,000+
- A100 40 GB PCIe (used, from German seller): ~€3,600 — only 40 GB VRAM

**The reality:** most A100 "PCIe" cards available in Europe are Chinese SXM4-to-PCIe conversions. No official warranty, no guarantee of quality, and if it fails you have no recourse. A real original A100 80 GB PCIe card is effectively €15,000+ in Europe.

These are Ampere-generation GPUs (2020) — two generations behind Blackwell. Still powerful, but no native FP4 support and no path to future optimizations.

### Option 3: Cloud computing

No upfront cost, pay per hour. Good for occasional training jobs or testing.

**But:** Costs accumulate quickly with regular use. An A100 80 GB on RunPod or JarvisLabs is ~€1.50-2.00/hour. At that rate, 100 hours of fine-tuning/experimentation already costs €150-200 — much less than a Spark. But for 2,000+ hours per year (which dedicated AI development can easily reach), the cost exceeds hardware purchase. Plus: data leaves the machine, sessions need setup, availability isn't guaranteed, and there's latency for interactive use.

Cloud makes most sense as a complement to local hardware — use local for iteration and experiments, burst to cloud for large training runs that need more compute than any local device provides.

### Option 4: DGX Spark (~€4,500 in NL)

All-in-one device. 128 GB unified memory, 240W total, near-silent, Blackwell architecture.

Available at DectDirect.nl for €4,486 (Founders Edition, 4TB SSD, March 2026). Also listed at Alternate.nl. NVIDIA raised the MSRP from $3,999 to $4,699 due to global memory supply shortages.

**But:** It's expensive for an individual. That's a significant investment that could instead be a year+ of cloud credits. The 273 GB/s unified bandwidth is slower than dedicated VRAM for models that fit on a GPU. And for pure GPU-compute tasks that need raw FLOPS (like image/video generation where the model fits in VRAM), a desktop GPU is faster.

OEM variants (ASUS Ascent GX10, Dell Pro Max with GB10, HP ZGX Nano) exist but EU availability and pricing remain unclear.

### Option 5: Full platform upgrade (AM5 + Pro 6000 96GB)

*v2 addition — this is the "ultimate" option from my upgrade research.*

A complete platform rebuild: AMD Ryzen 9 9950X3D (~€699), X870E motherboard (~€399-429), 128 GB+ DDR5 RAM (~€1,450+ at current crisis pricing), plus an RTX Pro 6000 Blackwell 96 GB (~€9,500+). Total estimated cost: ~€13,500, with ~€2,250 back from selling existing CPU/mobo/RAM. Net: ~€11,250.

This would give 96 GB VRAM at full GPU bandwidth (~1.8+ TB/s) plus 128 GB DDR5 for offloading — up to 224 GB total memory, of which 96 GB runs at GPU speed. In every dimension superior to all other options.

**But:** At €11,250 net, it's not financially realistic right now. DDR5 64 GB modules have risen sharply in price (the Corsair 2×64 GB 6400 kit was estimated at €1,018 but is now €1,450+ where available, and often out of stock entirely). The Pro 6000 96 GB itself is barely available and priced above €9,500. This is a long-term goal, not a short-term option. See [My upgrade path](#my-upgrade-path) for timing.

### Investigated: Server build (EPYC + A100)

*v2 addition — investigated and rejected on 10 March 2026.*

I explored whether a self-built AI server on a used EPYC platform with a used NVIDIA A100 80 GB would offer better value than a DGX Spark at the €12,000-15,000 price range.

**Real prices found on eBay.de (March 2026):**

| Component | Price | Note |
|-----------|-------|------|
| A100 80 GB PCIe (SXM4→PCIe conversion, China) | ~€7,600 | No warranty, converted card |
| A100 80 GB PCIe (original, new) | €15,000–18,000+ | Barely available |
| A100 80 GB SXM4 (refurbished) | ~€6,200 | Requires HGX baseboard (~€3,000+) |
| A100 40 GB PCIe (used, DE) | ~€3,600 | Only 40 GB VRAM |
| EPYC platform + 256 GB ECC RAM + PSU + case | ~€2,000–3,000 | Used datacenter surplus |

**Total realistic cost:** €10,000–12,000+ for 80 GB VRAM without warranty, on a two-generation-old platform.

**Why it was rejected:**
- **Price vs capacity:** €10,000+ for 80 GB without warranty. DGX Spark offers 128 GB unified for €4,500 with warranty.
- **Supply quality:** Almost all EU supply is Chinese SXM4-to-PCIe conversions. No official warranty, unclear provenance.
- **Outdated platform:** A100 is Ampere (2020), two generations behind Blackwell.
- **Noise:** Passively cooled server GPU requires rackmount chassis with high-RPM fans.
- **Power:** A100 draws ~300W, platform ~200W extra. Vs ~100W total for the Spark.
- **No gaming:** Dedicated AI machine, just like the Spark, but more expensive and more complex.

**Verdict:** Not realistic for a consumer in the Netherlands/Belgium. The DGX Spark offers more memory, less risk, and less complexity for less money. The server route only makes sense for someone with access to datacenter surplus and a dedicated server room.

### Comparison at a glance

| Factor | Add RTX 5090 | Used A100 80GB PCIe | Cloud | DGX Spark | AM5 + Pro 6000 | ~~Server build~~ |
|--------|-------------|---------------------|-------|-----------|----------------|-------------------|
| Cost | €3,000-4,000 | €7,600+ (no warranty) | €1.50/hr+ | ~€4,500 | ~€11,250 net | ~~€10,000-12,000+~~ |
| Total memory | 48-56 GB VRAM | 80 GB HBM2e | 40-80 GB (rental) | 128 GB unified | 96 GB VRAM + 128 GB RAM | ~~80 GB + 256 GB ECC~~ |
| Bandwidth | ~1.8 TB/s (5090) | ~2 TB/s (A100) | Varies | 273 GB/s | ~1.8+ TB/s (Pro 6000) | ~~~2 TB/s~~ |
| Power | +575W GPU alone | +300W GPU | N/A | 240W total | 500-800W system | ~~500W+~~ |
| Training headroom | Limited (LoRA ≤13B) | Good (80 GB) | Good | Good (128 GB) | Excellent (96+128 GB) | ~~Good (80 GB)~~ |
| Multi-model workflows | Still tight | Better | Flexible | Best (128 GB pool) | Excellent | ~~Good~~ |
| Privacy/offline | Yes | Yes | No | Yes | Yes | ~~Yes~~ |
| Noise | Significant | Significant | N/A | Near-silent | Normal desktop | ~~Server-loud~~ |
| Risk | None (new) | High (no warranty) | Vendor lock-in | None (new, NVIDIA warranty) | None (new) | ~~High~~ |
| Gaming | Via existing GPUs | No | No | No | Yes (top-tier) | ~~No~~ |
| Realistic now? | ⚠️ Scarce | ❌ Too expensive/risky | ✅ Always | ✅ Available | ❌ Too expensive | ~~❌ Rejected~~ |

---

## My upgrade path

*v2 addition — placing the Spark in the context of my overall hardware strategy.*

Based on extensive research into current pricing, availability, and my actual AI workloads, the most likely path forward is:

| When | Action | Net cost |
|------|--------|----------|
| **Short term** | DGX Spark — solve the immediate AI bottleneck | ~€4,500 |
| **~2 years** | PC platform upgrade (AM5 + DDR5, possibly with GPU upgrade) when hardware prices normalize | ~€2,750–6,500 |
| **Long-term goal** | Pro 6000 96 GB or successor — ultimate single-GPU solution | ~€9,500+ (GPU alone) |

**Why this order:**
- The Spark solves the most pressing problem *now*: models and training that don't fit or must be heavily compromised on the current desktop
- The gaming PC is perfectly fine for gaming and for inference on models that fit in VRAM — no urgent need to rebuild
- DDR5 64 GB modules are in a price crisis (~€1,450+ vs ~€1,018 estimated). Waiting makes financial sense
- The Spark and a future PC upgrade are financially separated in time — one doesn't block the other
- The Spark holds its value as a dedicated AI machine even after a PC upgrade (it becomes the quiet always-on inference server while the PC handles compute-heavy tasks)

**What if the Spark becomes obsolete?** The Spark uses Blackwell architecture with full CUDA support — the same ecosystem as desktop GPUs. Software written for it will continue to work. The 128 GB memory ceiling is generous for current models. Even if a faster Spark successor appears, the current one would remain useful as a secondary inference server. The risk is less "will it become useless" and more "will I wish I'd waited for something better" — but that's true of any hardware purchase.
...
│   ├── Small LLM models (≤7B) with LoRA?
│   │   └── DESKTOP GPU works fine (fits in 24 GB VRAM).
│   │
│   ├── Medium LLM models (7-13B) or aggressive LoRA?
│   │   └── SPARK or CLOUD — desktop VRAM is too tight.
│   │       Spark for regular use, cloud for occasional jobs.
│   │
│   ├── Large LLM models (>13B) or full fine-tuning?
│   │   └── CLOUD or SPARK — nothing else is practical at consumer prices.
│   │       Spark for privacy and regular use. Cloud for one-off jobs.
│   │
│   └── Video model training (LTX2 LoRA, etc.)?
│       ├── Current compromises (FP8/512p) are acceptable?
│       │   └── KEEP DESKTOP. It works, just slowly and at lower quality.
│       │
│       └── Want FP16/720p without compromises?
│           └── SPARK or RTX 5090.
│               Spark: everything fits, slower compute.
│               5090: faster compute, but still limited VRAM (32 GB).
│               Neither is perfect — the 5090 is faster when it fits,
│               the Spark handles bigger configurations.
│
...
│       IF you can find one — availability in NL is still poor (March 2026).
│
└── Want the "ideal" solution regardless of cost?
    └── RTX Pro 6000 Blackwell 96 GB in an AM5 desktop
        96 GB VRAM at full speed + 128 GB DDR5 = 224 GB total.
        Fastest AND enough memory. But ~€11,250 net.
        At that price, a Spark at €4,500 is a much more accessible
        first step that doesn't preclude this upgrade later.
```
...
8. **The server alternative is not viable.** A self-built EPYC + A100 80GB server costs €10,000-12,000+ in Europe for 80 GB VRAM without warranty, on an outdated platform. The DGX Spark offers more memory (128 GB), warranty, silence, and lower power consumption for less than half the price. The server route only works if you have datacenter access and are comfortable with converted Chinese hardware without warranty.
