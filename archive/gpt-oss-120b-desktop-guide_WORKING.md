...
The speed barely changed despite 16x more context — token generation speed for MoE is dominated by expert weight transfer from CPU, not KV cache size. The 64k config has one fewer layer on GPU (16 vs 15... wait, actually 16 vs 15 — we added layers 10-11 to CUDA0 but dropped layer 16 from CUDA1) which explains the tiny speed decrease.
